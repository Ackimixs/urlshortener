...
int main()
{
    ...
    try {
        ...
        SQLite::Statement query(db, "CREATE TABLE IF NOT EXISTS urls (code TEXT, long_url TEXT)");
        query.exec();
    ...
    CROW_ROUTE(app, "/<string>").methods(crow::HTTPMethod::GET)([](const std::string code) {
        SQLite::Database db("test.db3", SQLite::OPEN_READWRITE|SQLite::OPEN_CREATE);
        SQLite::Statement query(db, "SELECT * FROM urls WHERE code = ?");
        query.bind(1, code);
